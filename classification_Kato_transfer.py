...
def train(epoch, best_val_loss, log, train_loader, eval_data):
    ...
    cor_train  = []
    class_data = {}
    loss_fn = nn.NLLLoss(ignore_index=num_states)
    ...
    with torch.no_grad():
        ...
        for strain in eval_data:
            ...
            if k == 0:
                loss_data.update({'n2_nll':nll_strain_eval, 'n2_corr':strain_corr_eval})
            elif k==1:
                loss_data.update({'npr1_nll':nll_strain_eval, 'npr1_corr':strain_corr_eval})
            elif k==2:
                loss_data.update({'Kato_nll':nll_strain_eval, 'Kato_corr':strain_corr_eval})
            k+=1
    print_train_valid(epoch, loss_data, file=log)
    #Save model and losses for best validation
    if np.mean(nll_tot_eval) < best_val_loss:
        pk.dump(loss_data, open(buf_file, "wb"))
    scheduler.step()
    return np.mean(nll_tot_eval), loss_data
...
cuda = True
n_hidden = 10
dynamic_graph = True # <bool>: <True> to allow changing weights each timestep.
save_folder = "logs/Graph Classification/Eval_Other_Worms_10"
    ...
    while i != num_worms+1:
        ...
        for perm in training_worms:
            ...
            if model_type == 'MLP':
                ...
                class_model = Graph_Attention(n_hidden, num_states, nodes,cat=cat, dynamic_graph=dynami_graph).to(device)
            class_args = {}
            torch.save(class_model.state_dict(), class_init)
