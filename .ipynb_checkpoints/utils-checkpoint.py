...
def generate_data_lists(datasets):
    ...
    for worm_index in worms:
        ...
        train_valid_worm.update({worm_index+1: data[:-test_size]})
        test_worm.update({worm_index+1: data[-test_size:]})
...
def print_test_NRI(losses, pickle_file, file=sys.stdout):
    data = load_pickle(pickle_file)
    print('--------Testing-----------------',file=file)
    string = ''
    for key, value in losses.items():
        print(key + ': {} '.format(value), file=file)

    print('--------Best Epoch-------------',file=file)
    print('Best Epoch: {:04d}'.format(data['epoch']),
            'mse_train: {}'.format(np.around(np.mean(data['mse_train']),5)),
            'mse_val: {}'.format(np.around(np.mean(data['mse_val']),5)), file=file)

def print_test_class(losses, pickle_file, file=sys.stdout):
    data = load_pickle(pickle_file)
    print('--------Testing-----------------',file=file)
    string = ''
    for key, value in losses.items():
    ...
    print('--------Best Epoch-------------',file=file)
    print('Best Epoch: {}'.format(data['epoch']),
            ' | nll_train: {:.2f}'.format(np.mean(data['nll_train'])),
            ' | nll_val: {:.2f}'.format(np.mean(data['nll_val'])), file=file)
    

    return

def print_train(epoch, nll_train, mse_train, nll_val, mse_val, t, end, file=sys.stdout):
    print('Epoch: {:04d}'.format(epoch),
            'nll_train: {:.2f}'.format(np.mean(nll_train)),
            'mse_train: {:.5f}'.format(np.mean(mse_train)),
            'nll_val: {:.2f}'.format(np.mean(nll_val)),
            'mse_val: {:.5f}'.format(np.mean(mse_val)),
            'time: {:.4f}s\n'.format(time.time() - t), file=file, end=end)
    file.flush()
    return

def print_test(nll_test, mse_test, mse_str, pickle_file, file=sys.stdout):
    data = load_pickle(pickle_file)
    print('--------Testing-----------------',file=file)
    print('nll_test: {:.10f}'.format(np.mean(nll_test)),
          'mse_test: {:.10f}'.format(np.mean(mse_test)),file=file)
    print('MSE: {}\n'.format(mse_str),file=file)
    print('--------Best Epoch-------------',file=file)
    print('Best Epoch: {:04d}'.format(data['epoch']),
            'nll_train: {:.2f}'.format(np.mean(data['nll_train'])),
            'mse_train: {:.5f}'.format(np.mean(data['mse_train'])),
            'nll_val: {:.2f}'.format(np.mean(data['nll_val'])),
            'mse_val: {:.5f}'.format(np.mean(data['mse_val'])), file=file)
...
def save_file_NRI(folder_path, overwrite, args):
    abs_path = os.path.abspath('')
    path = os.path.join(abs_path, folder_path)
    folder_name = os.path.split(folder_path)[1]
    # If folder already exists, delete it if <overwrite> is <True>.
    ...
    if not os.path.exists(path):
        # Make folders and files.
        os.makedirs(path, exist_ok=True)
    enc_dec_init   = os.path.join(path, 'enc_dec_init.pt')
    enc_dec_best   = os.path.join(path, 'enc_dec_best.pt')
    log_file_NRI   = os.path.join(path, 'log.txt')
    buf_file_NRI   = os.path.join(path, 'buffer.pkl')
    edges_file     = os.path.join(path, 'edges.pkl')
    with open(log_file_NRI, 'w') as log:
        currentDT = datetime.datetime.now(tz=pytz.utc).astimezone(timezone('US/Pacific'))
        print('{}'.format(folder_name),file=log)
        print(currentDT.strftime("%Y-%m-%d: %I:%M:%S %p")+"\n",file=log)
        print("Save folder: {}".format(folder_path), file=log)
    return enc_dec_init, enc_dec_best, log_file_NRI, buf_file_NRI

# Empty logs folder (deletes all files/folders in <folder>)
# <folder> is path from notebook's directory to folder with logs. 
# For example, to delete all files in logs folder with path NRI/logs, use 'logs'.

def delete_logs(folder='logs', quiet = False):
    path = os.path.abspath('')
    parent, child=os.path.split(folder)
    parent = os.path.join(path, parent)
    assert child in os.listdir(parent), "<folder> not in directory"
    path = os.path.join(path, folder)
    del_fd = []
    failed_fd = []
    for fd in os.listdir(path):
        path_fd = os.path.join(path,fd)
        try:
            if os.path.isdir(path_fd):
                shutil.rmtree(path_fd)
            else:
                os.remove(path_fd)
            if not quiet:
                print("'%s' removed successfully" %fd)
        except:
            print ("Unexpected error:", sys.exc_info()[0])
    if not quiet:
        print("Done")
...
# Load meta file and return a dictionary with arguments.
...
def check_memory():
    ...
    # Allocation is currently used and reserved is cached.
    assert torch.cuda.is_available
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print('Cuda Memory Allocated (Gb): ', torch.cuda.memory_allocated(device)*1e-9)
    print('Cuda Memory Reserved (Gb): ', torch.cuda.memory_reserved(device)*1e-9)
    print('Cuda Max Allocated (Gb): ', torch.cuda.max_memory_allocated(device)*1e-9)
    print('Cuda Max Reserved (Gb): ', torch.cuda.max_memory_reserved(device)*1e-9)
    return
